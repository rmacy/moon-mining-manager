...
<div class="row" >
    <div class="col-12">
        ...
        <p>Lists ore volumes for each detonation, updates daily.</p>

        <p>
            @foreach (['Tax' => $corporationTax, 'Rent' => $corporationRent] as $name => $id)
            @if ($id == $corporationId) <strong> @endif
                <a href="/extractions?corporation={{$id}}">{{$name}} Corporation</a>
            @if ($id == $corporationId) </strong> @endif
            @endforeach
        </p>
        ...
        <p>
            Moon:
            <select onchange="extractionsReloadPage(this)" data-corporation-id="{{$corporationId}}">
        ...
        <p>
            ...
            @for ($i = 1; ($i - 1) * $limit < $total; $i++)
                @if ($i == $page) <strong> @endif
                    <a href="/extractions?corporation={{$corporationId}}&page={{$i}}&moon={{$moonId}}">&nbsp;{{$i}}&nbsp;</a>&nbsp;
                @if ($i == $page) </strong> @endif
            @endfor
        </p>
        ...
        <table>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{number_format($extraction->ore1_mined)}}</td>
                    <td>{{round($extraction->ore1_mined / $extraction->ore1_volume * 100, 2)}}</td>
                    <td data-id="{{$extraction->ore2_type_id}}">{{$extraction->ore2->typeName}}</td>
                    <td>{{number_format($extraction->ore2_volume)}}</td>
                    <td>{{number_format($extraction->ore2_mined)}}</td>
                    <td>{{round($extraction->ore2_mined / $extraction->ore2_volume * 100, 2)}}</td>
                    ...
                    <td>{{$extraction->ore3 ? number_format($extraction->ore3_mined) : ''}}</td>
                    <td>{{$extraction->ore3 ?
                        round($extraction->ore3_mined / $extraction->ore3_volume * 100, 2) : ''}}</td>
                    ...
                    <td>{{$extraction->ore4 ? number_format($extraction->ore4_mined) : ''}}</td>
                    <td>{{$extraction->ore4 ?
                            round($extraction->ore4_mined / $extraction->ore4_volume * 100, 2) : ''}}</td>
                </tr>
                @endforeach
            </tbody>
        </table>
...
function extractionsReloadPage(selectElement) {
    ...
    self.location.href = '/extractions?corporation='+corporationId+'&moon='+moonId;
}
</script>
